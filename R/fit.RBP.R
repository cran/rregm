fit.RBP<-function (formula = formula(data), sigma.formula = ~1, data, 
    param = "AM") 
{
    ...
    E.step <- function(zeta, data, param = "AM") {
        tau1 = switch(param, AM = 0, GM = 0.5, HM = 1, MO = 1, 
            MD = 1/3)
        tau2 = switch(param, AM = 0, GM = 0.5, HM = 1, MO = 2, 
            MD = 2/3)
        p = ncol(x)
        q = ncol(z)
        beta <- zeta[1:p]
    ...
    aux.M.step <- function(zeta, data, w1, logw, param) {
        ...
        a <- mu * sigma + tau1
        b <- sigma - tau2 + 1
        -sum(-lgamma(a) - lgamma(b) - (a + b) * logw - w1 * (y + 
            1) + (a - 1) * log(y))
    }
    M.step <- function(zeta, data, w1 = w1, logw = logw, param = param) nlminb(zeta, 
        aux.M.step, data = data, w1 = w1, logw = logw, param = param)$par
    ...
    q = ncol(z)
    aa = lm(log(y) ~ 1)
	if(ncol(x)>1) aa = lm(log(y) ~ x[, -1, drop = FALSE])
    ...
    while (dif > prec && i <= max.iter) {
        ...
        w1 = aux$w1
        logw = aux$logw
        zeta.new <- M.step(zeta, data.aux, w1 = w1, logw = logw, 
            param = param)
    ...
    se = sqrt(diag(solve(hessian(llike.BP, x0 = zeta, data = data.aux, 
        param = param))))
    aa = cbind(zeta, se)
    ...
    if (all(beta > 2)) {
        mean.y = alpha/(beta - 1)
        sd.y = sqrt(alpha * (alpha + beta - 1)/((beta - 2) * 
            (beta - 1)^2))
    ...
    val = list(estimate = aa, logLik = llike, AIC = -2 * llike + 
        2 * (p + q), BIC = -2 * llike + log(length(y)) * (p + 
        q), tau1 = tau1, tau2 = tau2, pearson.res = as.vector(pearson), 
        mod.pearson.res = as.vector(mod.pearson), quant.res = c(quant), 
        convergence = convergence, dist = "BP", param = param, 
        mu.x = x, sigma.x = z)
    class(val) <- "rregm"
    val
}
