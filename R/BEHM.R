...
{
    mstats <- checklink("mu.link", "BetaHM", substitute(mu.link), 
        c("logit", "probit", "cloglog", "cauchit", "log", "own"))
    dstats <- checklink("sigma.link", "BetaHM", substitute(sigma.link), 
        c("log", "inverse", "identity", "own"))
    structure(list(family = c("BEHM", "BetaHM"), parameters = list(mu = TRUE, 
...
c1<-1; c2<-1
            ...
            d2ldmdd
        }, G.dev.incr = function(y, mu, sigma, w, ...) -2 * dBEHM(y, 
            mu, sigma, log = TRUE), rqres = expression(rqres(pfun = "pBEHM", 
...
mean = function(mu, sigma){c1<-1; c2<-1;(mu*sigma+c1)/(sigma+c1+c2)}, 
variance = function(mu, sigma){c1<-1; c2<-1;(mu*sigma+c1)*((1-mu)*sigma+c2)/((sigma+c1+c2)^2*(sigma+1+c1+c2))}), 
