...
function (mu.link = "log", sigma.link = "log") 
{
  ...
  dstats <- checklink("sigma.link", "Gamma", substitute(sigma.link), 
                      c("inverse", "log", "identity", "own"))
  structure(list(family = c("IGAM", "Inverse_gamma_arithmetic_mean"), 
                 ...
                 G.dev.incr = function(y, mu, sigma, w, 
                                       ...) -2 * dIGAM(y, mu, sigma, log = TRUE), rqres = expression(rqres(pfun = "pIGAM", 
                 ...
                 variance   = function(mu, sigma) (mu^2)/(sigma-1)
