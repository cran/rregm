BEGM<-function (mu.link = "logit", sigma.link = "log") 
{
    ...
    structure(list(family = c("BEGM", "BetaGM"), parameters = list(mu = TRUE, 
        ...
        sigma.dr = dstats$mu.eta, dldm = function(y, mu, sigma) {
            ...
            a <- mu * sigma + c1
            b <- (1 - mu) * sigma + c2 - c1
            dldm <- sigma * (log(y) - log1p(-y) - digamma(a) + 
                digamma(b))
            dldm
            ...
            a <- mu * sigma + c1
            b <- (1 - mu) * sigma + c2 - c1
            dldd <- mu * (log(y) - log1p(-y) - digamma(a) + digamma(b)) + 
                log1p(-y) - digamma(b) + digamma(a + b)
            dldd
            ...
            a <- mu * sigma + c1
            b <- (1 - mu) * sigma + c2 - c1
            d2ldd2 <- -mu^2 * (trigamma(a) + trigamma(b)) + trigamma(b) * 
                (2 * mu - 1) + trigamma(a + b)
            d2ldd2
    ...
    sigma*
      (mu*trigamma(a) -
       (1-mu)*trigamma(b))
        ...
        }), mu.valid = function(mu) all(mu > 0 & mu < 1), sigma.valid = function(sigma) all(sigma > 
            0), y.valid = function(y) all(y > 0 & y < 1), mean = function(mu, 
            sigma) {
            c1 <- 0.5
            c2 <- 0.5
            (mu * sigma + c1)/(sigma + c1 + c2)
        }, variance = function(mu, sigma) {
            c1 <- 0.5
            c2 <- 0.5
            (mu * sigma + c1) * ((1 - mu) * sigma + c2)/((sigma + 
                c1 + c2)^2 * (sigma + 1 + c1 + c2))
